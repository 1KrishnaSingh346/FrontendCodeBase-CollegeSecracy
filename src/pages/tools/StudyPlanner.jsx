...
const StudyPlanner = () => {
  ...
  useEffect(() => {
    const savedSessions = getFromLocalStorage('studySessions');
    const savedCompleted = getFromLocalStorage('completedSessions');
    
    if (Array.isArray(savedSessions)) setStudySessions(savedSessions);
  ...
  const toggleBookmark = (id) => {
    const updatedSessions = studySessions.map(session => 
      session.id === id ? { ...session, bookmarked: !session.bookmarked } : session
    );
    setStudySessions(updatedSessions);
    
    if (selectedSession?.id === id) {
      setSelectedSession(prev => ({ ...prev, bookmarked: !prev.bookmarked }));
    }
  };
  ...
  const filteredSessions = studySessions.filter(session => {
    ...
    return matchesSearch && matchesPriority && matchesStatus && matchesBookmarked;
  });
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex mb-6 border-b border-gray-200 dark:border-gray-700">
          <button
            ...
            className={`px-4 py-2 font-medium ${activeTab === 'planner' ? 'border-b-2 border-green-500 text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}
          >
            Planner
          </button>
          <button
            onClick={() => setActiveTab('history')}
            className={`px-4 py-2 font-medium ${activeTab === 'history' ? 'border-b-2 border-green-500 text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}
          >
            History
          </button>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-gray-200 dark:border-gray-700">
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 text-green-700 dark:text-green-400">
                {editMode ? <FiEdit2 className="w-5 h-5" /> : <FiPlus className="w-5 h-5" />}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    name="title"
                    value={editMode ? selectedSession?.title || '' : newSession.title}
                    onChange={editMode ? (e) => setSelectedSession({...selectedSession, title: e.target.value}) : (e) => setNewSession({...newSession, title: e.target.value})}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="subject"
                    value={editMode ? selectedSession?.subject || '' : newSession.subject}
                    onChange={editMode ? (e) => setSelectedSession({...selectedSession, subject: e.target.value}) : (e) => setNewSession({...newSession, subject: e.target.value})}
                    className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                ...
                <div>
                  ...
                  <input
                    type="date"
                    name="date"
                    value={editMode ? selectedSession?.date || '' : newSession.date}
                    onChange={editMode ? (e) => setSelectedSession({...selectedSession, date: e.target.value}) : (e) => setNewSession({...newSession, date: e.target.value})}
                    className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    required
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="time"
                      name="startTime"
                      value={editMode ? selectedSession?.startTime || '' : newSession.startTime}
                      onChange={editMode ? (e) => setSelectedSession({...selectedSession, startTime: e.target.value}) : (e) => setNewSession({...newSession, startTime: e.target.value})}
                      className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="time"
                      name="endTime"
                      value={editMode ? selectedSession?.endTime || '' : newSession.endTime}
                      onChange={editMode ? (e) => setSelectedSession({...selectedSession, endTime: e.target.value}) : (e) => setNewSession({...newSession, endTime: e.target.value})}
                      className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                    />
                  </div>
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    name="topics"
                    value={editMode ? selectedSession?.topics || '' : newSession.topics}
                    onChange={editMode ? (e) => setSelectedSession({...selectedSession, topics: e.target.value}) : (e) => setNewSession({...newSession, topics: e.target.value})}
                    className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                ...
                <div>
                  ...
                  <select
                    name="priority"
                    value={editMode ? selectedSession?.priority || 'medium' : newSession.priority}
                    onChange={editMode ? (e) => setSelectedSession({...selectedSession, priority: e.target.value}) : (e) => setNewSession({...newSession, priority: e.target.value})}
                    className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                  >
                ...
                <div>
                  ...
                  <select
                    name="status"
                    value={editMode ? selectedSession?.status || 'planned' : newSession.status}
                    onChange={editMode ? (e) => setSelectedSession({...selectedSession, status: e.target.value}) : (e) => setNewSession({...newSession, status: e.target.value})}
                    className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                  >
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    name="notes"
                    value={editMode ? selectedSession?.notes || '' : newSession.notes}
                    onChange={editMode ? (e) => setSelectedSession({...selectedSession, notes: e.target.value}) : (e) => setNewSession({...newSession, notes: e.target.value})}
                    className="w-full p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-green-500 focus:border-green-500"
                ...
                {editMode && (
                  <div className="md:col-span-2 flex items-center">
                    <input
                      ...
                      onChange={() => setSelectedSession(prev => ({ ...prev, bookmarked: !prev.bookmarked }))}
                      className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                    />
                    <label htmlFor="bookmarked" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                    />
                    <label htmlFor="bookmarked-filter" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                      Bookmarked only
                    </label>
                  </div>
                </div>
              ...
              {filteredSessions.length === 0 ? (
                <div className="text-center py-8 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                  <p className="text-gray-500 dark:text-gray-400">
                    {searchTerm || filters.priority !== 'all' || filters.status !== 'all' || filters.bookmarked 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredSessions.map(session => (
                    <div 
                      key={session.id}
                      className={`p-5 rounded-xl border ${priorityBorderColors[session.priority]} shadow-sm hover:shadow-md transition-all cursor-pointer ${
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {session.subject && (
                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{session.subject}</p>
                          )}
                          ...
                          <div className="mt-3 space-y-1.5 text-sm">
                            <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                              ...
                              <span>
                                {formatDate(session.date)} • {daysUntilSession(session.date)}
                              </span>
                            </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            <FiBookmark className={`w-4 h-4 ${session.bookmarked ? 'fill-current' : ''}`} />
                          </button>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                      ...
                      {session.topics && (
                        <div className="mt-3">
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Topics:</p>
                  ...
                  >
                    ...
                    <div className="mt-3 space-y-2 text-sm text-gray-700 dark:text-gray-300">
                      <div className="flex items-center gap-2">
                        ...
                        <span>Priority: <span className="capitalize">{session.priority}</span></span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>Completed: {formatDate(session.completionDate)}</span>
                      </div>
                      ...
                      {session.topics && (
                        <div>
                          <p className="font-medium text-gray-500 dark:text-gray-400">Topics covered:</p>
                          <p className="text-gray-700 dark:text-gray-300">{session.topics}</p>
                        </div>
                      )}
